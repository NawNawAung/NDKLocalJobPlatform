            ...
            <section class="my-auto w-full max-w-[540px] rounded-2xl bg-white p-6 shadow-2xl sm:p-8" role="dialog" aria-modal="true" aria-labelledby="interview-modal-title">
                ...
                <form class="mt-6" @submit.prevent="submit">
                    ...
                    <div class="mt-5 grid gap-4 sm:grid-cols-2">
                        <label class="block"><span class="form-label">Date</span><span class="relative block"><i class="ti ti-calendar pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" aria-hidden="true"/><input v-model="form.date" required type="date" :min="dateTimeMin.slice(0, 10)" class="form-field !pl-10"/></span></label>
                        <label class="block"><span class="form-label">Time <span class="font-normal text-slate-500">(Myanmar time)</span></span><span class="relative block"><i class="ti ti-clock pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" aria-hidden="true"/><input v-model="form.time" required type="time" :min="form.date === dateTimeMin.slice(0, 10) ? dateTimeMin.slice(11, 16) : undefined" class="form-field !pl-10"/></span></label>
                    </div>
