...
async function loadJob() {
    ...
    try {
        ...
        isOwnListing.value = Boolean(data.is_own_listing);
                ...
                <div class="mt-5 grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_310px]">
                    <div class="space-y-5">
                        <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8">
                            ...
                            <div class="mt-6 flex flex-wrap gap-3"><button v-if="role !== 'employer'" type="button" :disabled="hasApplied" class="rounded-lg bg-[var(--brand-primary)] px-5 py-3 text-sm font-semibold text-white shadow-sm hover:bg-[var(--brand-primary-hover)] disabled:cursor-default disabled:opacity-70" @click="scrollToApplication">{{ hasApplied ? 'Application submitted' : 'Apply for this job' }}</button><button v-if="role !== 'employer'" type="button" :disabled="saving" class="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50" @click="toggleSaved"><i :class="saved ? 'ti ti-bookmark-filled text-blue-700' : 'ti ti-bookmark'" aria-hidden="true"/>{{ saved ? 'Saved' : 'Save job' }}</button><a v-if="role === 'employer' && isOwnListing" href="/#dashboard" class="inline-flex items-center gap-2 rounded-lg bg-[var(--brand-primary)] px-4 py-3 text-sm font-semibold text-white hover:bg-[var(--brand-primary-hover)]"><i class="ti ti-briefcase" aria-hidden="true"/>Manage your listing</a><button type="button" class="inline-flex items-center gap-2 rounded-lg border border-slate-300 px-4 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50" @click="shareJob"><i class="ti ti-share" aria-hidden="true"/>Share</button></div>
                            <p v-if="role === 'employer'" class="mt-3 rounded-lg bg-blue-50 px-4 py-3 text-sm leading-6 text-blue-950">{{ isOwnListing ? 'This is one of your company’s listings. Manage its status and applications from your employer dashboard.' : 'Market reference listing published by another employer. Applications are available to job seekers.' }}<a v-if="isOwnListing" href="/#dashboard" class="ml-1 font-semibold underline">Open employer dashboard</a></p>
                        ...
                        <article v-if="role !== 'employer'" ref="applicationForm" class="scroll-mt-24 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-8"><h2 class="text-lg font-bold text-slate-900">Apply for {{ job.title }}</h2><p class="mt-1 text-sm text-slate-600">Your application and contact details will be shared with this employer.</p>
